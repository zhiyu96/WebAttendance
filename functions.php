<?php 
	...
	// connect to database
	$db = mysqli_connect('localhost', 'root', 'test123', 'login');
	...
	function register(){
		...
		if (count($errors) == 0) {
			...
			if (isset($_POST['ProfileType'])) {
				...
				$query = "INSERT INTO profile (Name, MatricNo, TagID, ProfileType, Email, PhoneNo, Username, Password) 
						  VALUES('$name', '$matric', '$tagid', '$account', '$email', '$phone', '$username', '$password')";
				mysqli_query($db, $query);
				$_SESSION['success']  = "New user successfully created!!";
				header('location: home.php');
			}else{
				$query = "INSERT INTO profile (Name, MatricNo, TagID, ProfileType, Email, PhoneNo, Username, Password) 
						  VALUES('$name', '$matric', '$tagid', 'user', '$email', '$phone', '$username', '$password')";
				mysqli_query($db, $query);
		...
		if (count($errors) == 0) {
			...
			if (mysqli_num_rows($results) == 1) { // user found
				...
				if ($logged_in_user['ProfileType'] == 'admin') {
					...
					$_SESSION['user'] = $logged_in_user;
					$_SESSION['success']  = "You are now logged in";
					header('location: lecturer/lechome.php');		  
				}else {
					$_SESSION['user'] = $logged_in_user;
					$_SESSION['success']  = "You are now logged in";
